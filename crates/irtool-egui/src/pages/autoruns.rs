...
use irtool_service::types::{AutorunItem, RiskLevel, ScanOptions, ScanPhase, ScanProgress, SignatureStatus};
...
impl AutorunsPageState {
    ...
    pub fn handle_scan_progress(&mut self, progress: ScanProgress) {
        ...
        self.scanning = !matches!(
            self.scan_progress.as_ref().and_then(|p| Some(&p.phase)),
    ...
    fn render_toolbar(&mut self, ui: &mut egui::Ui, ctx: &AppContext, rt: &tokio::runtime::Handle) {
        ui.horizontal(|ui| {
            ...
            egui::ComboBox::from_id_salt("autoruns_cat_filter")
        ...
        if items.is_empty() {
            ...
            ui.vertical_centered(|ui| {
                if self.items.is_empty() {
                    ui.label(
                        egui::RichText::new("点击「开始扫描」检测持久化项")
                            .color(theme::FG_SECONDARY),
                    );
                } else {
                    ui.label(
                        egui::RichText::new("没有匹配当前过滤条件的条目")
        ...
        egui::ScrollArea::both()
            ...
            .show(ui, |ui| {
                let table = TableBuilder::new(ui)
                    ...
                    .column(Column::initial(40.0))   // 启用
                    .column(Column::initial(100.0))  // 类别
                    .column(Column::initial(80.0))   // 签名
                    .column(Column::initial(200.0))  // 条目
                    .column(Column::initial(280.0))  // 文件路径
                    .column(Column::initial(240.0))  // 启动命令
                    .column(Column::initial(140.0)); // 发布者
                ...
                table
                    ...
                    .body(|body| {
                        body.rows(theme::TABLE_ROW_HEIGHT, items.len(), |mut row| {
                            ...
                            row.col(|ui| {
                                ui.label(
                                    egui::RichText::new(item.image_path.as_deref().unwrap_or(""))
                            ...
                            row.col(|ui| {
                                ui.label(
                                    egui::RichText::new(item.launch_string.as_deref().unwrap_or(""))
                                        .font(egui::FontId::monospace(11.0))
                                        .color(theme::FG_TERTIARY),
                                );
                            });
                            row.col(|ui| { ui.label(&item.publisher); });
        ...
        egui::Area::new(egui::Id::new("autoruns_ctx_menu"))
            ...
            .show(ui.ctx(), |ui| {
                egui::Frame::popup(ui.style()).show(ui, |ui| {
                    ...
                    if ui.add_enabled(item.image_path.is_some(), egui::Button::new("计算哈希")).clicked() {
                        action = Some(CtxAction::CalcHash);
                        close_menu = true;
                    }
                    if ui.add_enabled(item.image_path.is_some(), egui::Button::new("Sigcheck")).clicked() {
                        action = Some(CtxAction::Sigcheck);
                        close_menu = true;
                    }
                    if ui.button("删除条目").clicked() {
    ...
    fn execute_ctx_action(&mut self, action: CtxAction, item: &AutorunItem, ctx: &AppContext, rt: &tokio::runtime::Handle) {
        match action {
            CtxAction::Copy => {
                let text = format!(
                    "{} {} {}",
                ...
                if let Some(ref path) = item.image_path {
                    ...
                    rt.spawn(async move {
                        match AutorunsService::sigcheck(path).await {
                            Ok(output) => tracing::info!("sigcheck output:\n{}", output),
                            Err(e) => tracing::error!("sigcheck failed: {}", e),
                        }
                        let _ = ctx_clone;
                    });
                }
            ...
            CtxAction::Delete => {
                ...
                rt.spawn(async move {
                    match (AutorunsService { ctx: &ctx_clone }).delete_entry(entry_id).await {
                        Ok(result) => tracing::info!("delete result: success={}, msg={}", result.success, result.message),
    ...
    pub fn render_detail_panel(&mut self, ui: &mut egui::Ui, ctx: &AppContext, rt: &tokio::runtime::Handle) {
        ...
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    ui.label("风险等级:");
                    risk_badge(ui, item.risk);
                });
                if !item.risk_reasons.is_empty() {
                    ui.add_space(2.0);
                ...
                ui.horizontal(|ui| {
                    ...
                    if ui.button("🗑 删除").clicked() {
                        let ctx_clone = ctx.clone();
                        rt.spawn(async move {
                            let _ = (AutorunsService { ctx: &ctx_clone }).delete_entry(entry_id).await;
                        });
                        self.selected_id = None;
                        self.selected_item = None;
                        self.detail_visible = false;
    ...
    pub fn render_stats_bar(&self, ui: &mut egui::Ui) {
        ui.horizontal(|ui| {
            ...
            let signed = self.items.iter().filter(|d| matches!(d.signature, SignatureStatus::Valid { .. })).count();
            ...
            if self.scanning {
                ...
                ui.label(
                    egui::RichText::new(format!("上次扫描: {} 项", count))
                        .color(theme::FG_TERTIARY)
                        .size(11.0),
                );
            }
    ...
    fn compute_filtered_sorted(&self) -> Vec<AutorunItem> {
        ...
        let mut out: Vec<AutorunItem> = self
            ...
            .filter(|item| {
                self.category_filter.is_empty() || self.category_filter.contains(&item.category)
            })
            .filter(|item| {
                if q.is_empty() {
                    return true;
        ...
        out.sort_by(|a, b| {
            let ord = match sc {
                ...
                SortColumn::ImagePath => {
                    a.image_path.as_deref().unwrap_or("").cmp(b.image_path.as_deref().unwrap_or(""))
                }
                SortColumn::LaunchString => {
                    a.launch_string.as_deref().unwrap_or("").cmp(b.launch_string.as_deref().unwrap_or(""))
                }
...
fn cell_enabled(ui: &mut egui::Ui, item: &AutorunItem) {
    let (text, color) = if item.enabled {
...
fn cell_signature(ui: &mut egui::Ui, sig: &SignatureStatus) {
    match sig {
        SignatureStatus::Valid { signer } => {
            let label = if signer.is_empty() { "已签名".to_string() } else { signer.clone() };
            ui.horizontal(|ui| {
                badge::badge(ui, "签", BadgeVariant::Success);
                ui.label(
                    egui::RichText::new(truncate_str(&label, 12))
                        .color(theme::FG_SECONDARY)
                        .size(11.0),
                );
            });
...
fn signature_badge(ui: &mut egui::Ui, sig: &SignatureStatus) {
    match sig {
        SignatureStatus::Valid { signer } => {
            ...
            if !signer.is_empty() {
                ui.label(
                    egui::RichText::new(format!("({})", truncate_str(signer, 20)))
                        .color(theme::FG_TERTIARY)
                        .size(10.0),
                );
            }
...
fn risk_badge(ui: &mut egui::Ui, risk: RiskLevel) {
    match risk {
        RiskLevel::Safe => badge::badge(ui, "安全", BadgeVariant::Success),
        RiskLevel::Suspicious => badge::badge(ui, "可疑", BadgeVariant::Warning),
        RiskLevel::HighRisk => badge::badge(ui, "高风险", BadgeVariant::Danger),
    }
}
...
fn detail_row(ui: &mut egui::Ui, label: &str, value: Option<&str>, mono: bool) {
    ...
    ui.horizontal(|ui| {
        ...
        let text = if mono { text.font(egui::FontId::monospace(11.0)) } else { text };
        ui.label(text);
    });
    ui.add_space(2.0);
}
