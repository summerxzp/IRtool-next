...
impl SettingsPageState {
    ...
    fn select_all_popup(&mut self, select: bool) {
        if select {
            self.config.notify_config.popup_rule_ids =
    ...
    fn select_all_feishu(&mut self, select: bool) {
        if select {
            self.config.notify_config.feishu_rule_ids =
                self.config.rules.iter().map(|r| r.id.clone()).collect();
        } else {
        ...
        ui.horizontal(|ui| {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ...
                if ui
                    .add(
                        egui::Button::new(
                            egui::RichText::new(if self.saving { "保存中…" } else { "保存" })
                                .color(theme::ACCENT),
                        ),
                    )
                    .clicked()
    ...
    fn render_notification(&mut self, ui: &mut egui::Ui, ctx: &AppContext, rt: &tokio::runtime::Handle) {
        ...
        egui::Frame::group(ui.style())
            ...
            .show(ui, |ui| {
                    ...
                    for rule in &self.config.rules {
                        ui.horizontal(|ui| {
                            ...
                            for et in &rule.event_types {
                                if let Some((_, label)) =
                                    EVENT_TYPE_OPTIONS.iter().find(|(k, _)| k == et)
                                {
                ...
                ui.horizontal(|ui| {
                    ui.label(
                        egui::RichText::new("弹窗时长")
                            .color(theme::FG_SECONDARY)
                            .size(11.0),
                    );
                    ui.add(
                        egui::DragValue::new(&mut self.config.notify_config.popup_duration_secs)
                            .range(0..=3600),
                    );
                    ui.label(
                    ...
                    Ok(imported) => {
                        ...
                        for rule in imported {
                            if !existing_ids.contains(&rule.id) {
                                self.targets_input
                                    .insert(rule.id.clone(), rule.targets.join(", "));
                                self.config.rules.push(rule);
                            }
                        }
                        if let Some(tx) = &self.refresh_tx {
